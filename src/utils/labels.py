...
def wrap_text_lines(text: str, max_width: float, measure_fn: Callable[[str], float]) -> List[str]:
    words = text.split()
    lines, current_line = [], ""
    for word in words:
...
def draw_pdf_text(
    ...
    if wrap:
        ...
        lines = wrap_text_lines(text, max_w_mm * mm, lambda t: pdf.stringWidth(t, font, size))
        for line in lines:
            px = (x_mm * mm if x_mm is not None else (PAGE_W_MM * mm - pdf.stringWidth(line, font, size)) / 2)
    ...
    while size > 4 and pdf.stringWidth(text, font, size) > (max_w_mm * mm):
        size -= 1
    pdf.setFont(font, size)
    px = (x_mm * mm if x_mm is not None else (PAGE_W_MM * mm - pdf.stringWidth(text, font, size)) / 2)
...
def _draw_single_stock_label_pdf(pdf: Canvas, data: dict, order: dict, qr_code: bool):
    ...
    if LOGO_PATH.exists():
        pdf.drawImage(str(LOGO_PATH), MARGIN_MM * mm, (PAGE_H_MM - 17) * mm, 10 * mm, 10 * mm, mask="auto")

    nfe = order.get("nfe", data.get("nfe_number", ""))
    draw_pdf_text(pdf, PAGE_H_MM - 7, f"NF {nfe}", max_w_mm=85, size=13)
    draw_pdf_text(pdf, PAGE_H_MM - 6, order.get("address", ""), x_mm=70, max_w_mm=15, font="Arial", size=5, wrap=True)
    draw_pdf_text(pdf, PAGE_H_MM - 15, str(order.get("order", "")), max_w_mm=85, size=11)
    ...
    draw_pdf_text(pdf, PAGE_H_MM - 37, order.get("code", ""), max_w_mm=85, font="Arial", size=16, color=white)
    draw_pdf_text(pdf, PAGE_H_MM - 45, order.get("description", ""), max_w_mm=82, font="Arial", size=8, wrap=True)
    draw_pdf_text(pdf, MARGIN_MM + 10 - MARGIN_MM, f"Quantidade: {int(order.get('qty', 0))} {order.get('unit_type', '')}", max_w_mm=85, font="Arial", size=10)
    draw_pdf_text(pdf, MARGIN_MM, f"Lote Total: {int(order.get('qty_total', 0))} {order.get('unit_type', '')}", max_w_mm=85, font="Arial", size=10)
    pdf.showPage()
...
def _draw_single_pending_label_pdf(pdf: Canvas, nfe_number: str, mat: dict):
    ...
    pdf.rect((PAGE_W_MM - 10) * mm, MARGIN_MM * mm, 10 * mm, (PAGE_H_MM - 2 * MARGIN_MM) * mm, stroke=0, fill=1)
    pdf.rect(5 * mm, 5 * mm, (PAGE_W_MM - 20) * mm, 10 * mm, stroke=0, fill=1)

    draw_pdf_text(pdf, PAGE_H_MM - 10, f"NF {nfe_number}", offset_x_mm=-5, max_w_mm=80, size=14, color=white)
    draw_pdf_text(pdf, PAGE_H_MM - 17, f"Tipo de Serviço: {mat.get('service_type', '')}", offset_x_mm=-5, size=14, color=white)
    draw_pdf_text(pdf, PAGE_H_MM - 28, mat.get("op_number", ""), offset_x_mm=-5, size=18, color=white)
    draw_pdf_text(pdf, PAGE_H_MM - 38, mat.get("product", ""), offset_x_mm=-5, font="Arial", size=18, color=white)
    draw_pdf_text(pdf, PAGE_H_MM - 50, mat.get("code", ""), offset_x_mm=-5, font="Arial", size=18, color=white)
    draw_pdf_text(pdf, 8, f"QUANTIDADE: {int(mat.get('pending_qty', 0))} UND", offset_x_mm=-5, font="Arial", size=10, color=black)
    pdf.showPage()
...
def draw_img_text(
    ...
    if wrap:
        ...
        lines = wrap_text_lines(text, x_px(max_w_mm), lambda t: draw.textlength(t, font=font))
        for line in lines:
            px = (x_px(x_mm) if x_mm is not None else (PAGE_W_PX - draw.textlength(line, font=font)) / 2)
            draw.text((px + x_px(offset_x_mm), y_px(PAGE_H_MM - y_mm)), line, font=font, fill=color, anchor="ls")
            y_mm -= 3.0
        return
    ...
    px = (x_px(x_mm) if x_mm is not None else (PAGE_W_PX - draw.textlength(text, font=font)) / 2)
    draw.text((px + x_px(offset_x_mm), y_px(PAGE_H_MM - y_mm)), text, font=font, fill=color, anchor="ls")

def _generate_single_stock_img(data: dict, order: dict, qr_code: bool, logo_img: Image, output_path: str):
    img = Image.new("RGB", (PAGE_W_PX, PAGE_H_PX), "white")
    draw = ImageDraw.Draw(img)

    draw.rectangle([0, y_px(30), PAGE_W_PX, y_px(40)], fill="black")

    draw_img_text(draw, PAGE_H_MM - MARGIN_MM - 2, data.get("date", ""), x_mm=MARGIN_MM, max_w_mm=80, size=10)
    ...
    draw_img_text(draw, PAGE_H_MM - 6, order.get("address", ""), x_mm=70, max_w_mm=15, font_name="Arial", size=5, wrap=True)
    draw_img_text(draw, PAGE_H_MM - 15, str(order.get("order", "")), max_w_mm=85, size=11)
    ...
    draw_img_text(draw, PAGE_H_MM - 37, order.get("code", ""), max_w_mm=85, font_name="Arial", size=16, color="white")
    draw_img_text(draw, PAGE_H_MM - 45, order.get("description", ""), max_w_mm=82, font_name="Arial", size=8, wrap=True)
    draw_img_text(draw, MARGIN_MM + 10 - MARGIN_MM, f"Quantidade: {int(order.get('qty', 0))} {order.get('unit_type', '')}", max_w_mm=85, font_name="Arial", size=10)
    draw_img_text(draw, MARGIN_MM, f"Lote Total: {int(order.get('qty_total', 0))} {order.get('unit_type', '')}", max_w_mm=85, font_name="Arial", size=10)
...
def _generate_single_pending_img(nfe_number: str, mat: dict, output_path: str):
    ...
    draw.rectangle([x_px(PAGE_W_MM - 10), y_px(MARGIN_MM), PAGE_W_PX, y_px(PAGE_H_MM - MARGIN_MM)], fill="white")
    draw.rectangle([x_px(5), y_px(PAGE_H_MM - 15), x_px(PAGE_W_MM - 15), y_px(PAGE_H_MM - 5)], fill="white")

    draw_img_text(draw, PAGE_H_MM - 10, f"NF {nfe_number}", offset_x_mm=-5, max_w_mm=80, size=14, color="white")
    draw_img_text(draw, PAGE_H_MM - 17, f"Tipo de Serviço: {mat.get('service_type', '')}", offset_x_mm=-5, size=14, color="white")
    draw_img_text(draw, PAGE_H_MM - 28, mat.get("op_number", ""), offset_x_mm=-5, size=18, color="white")
    draw_img_text(draw, PAGE_H_MM - 38, mat.get("product", ""), offset_x_mm=-5, font_name="Arial", size=18, color="white")
    draw_img_text(draw, PAGE_H_MM - 50, mat.get("code", ""), offset_x_mm=-5, font_name="Arial", size=18, color="white")
    draw_img_text(draw, 8, f"QUANTIDADE: {int(mat.get('pending_qty', 0))} UND", offset_x_mm=-5, font_name="Arial", size=10, color="black")
